#cs
	 _                  _                            
	| |    __ _ _______| |_ _ __ ___  __ _ _ __ ___  
	| |   / _` |_  / __| __| '__/ _ \/ _` | '_ ` _ \ 
	| |__| (_| |/ /\__ \ |_| | |  __/ (_| | | | | | |
	|_____\__,_/___|___/\__|_|  \___|\__,_|_| |_| |_|
...
Global Const $sVersion = "v1.10", $sGUITitle = "Lazstream " & $sVersion, $sIconPath=@TempDir&"\lazstream.ico"
...
Global Const $sSectionName = "Lazstream", $aIniSections[31] = ["IpAddr", "PriorityMode", "PriorityFactor", "ImageQuality", _
...
AutoItSetOption("TrayAutoPause", 0)
FileInstall("res\lazstream.ico",$sIconPath,1)
...
If (IsAdmin() == 1 And @Compiled == 1) And (StringLeft(@OSVersion, 6) <> "WIN_XP" And @OSVersion <> "WIN_2003") Then ;This feature is not supported in Windows XP/XPe/Server 2003
	LogLine("WARNING: Lazstream was ran as admin and will now allow itself in Windows Firewall.", 1)
	If FileExists(@SystemDir & "\netsh.exe") Then
		ShellExecute(@SystemDir & '\netsh', 'advfirewall firewall delete rule name="Lazstream"', Default, Default, @SW_HIDE)
		ShellExecute(@SystemDir & '\netsh', 'advfirewall firewall add rule name="Lazstream" dir=in action=allow program="' & @ScriptFullPath & '" enable=yes', Default, Default, @SW_HIDE)
		ShellExecute(@SystemDir & '\netsh', 'advfirewall firewall add rule name="Lazstream" dir=out action=allow program="' & @ScriptFullPath & '" enable=yes', Default, Default, @SW_HIDE)
		LogLine("Done. It's strongly recommended to run Lazstream as a normal user from now on.", 1)
	Else
...
If $__g_hD2D1DLL == -1 Then
	MsgBox($MB_ICONERROR, "Direct2D init error", "There was an error while initializing Direct2D." & @CRLF & @CRLF & "This error is most likely caused by using Lazstream on an OS" & _
...
Func CreateMainGUIandSettings()
	...
	#Region ### START Koda GUI section ### Form=\gui\Lazstreamgui.kxf
	Global $LazstreamGUI = GUICreate($sGUITitle, 459, 202)
	...
	Global $GUI_IpAddr = _GUICtrlIpAddress_Create($LazstreamGUI, 104, 24, 105, 17)
	...
	GUISetOnEvent($GUI_EVENT_CLOSE, "ExitMain", $LazstreamGUI)
...
Func SendNFC()
	...
	If $iMsgBoxAnswer == -1 Then $iMsgBoxAnswer = MsgBox(67, "Choose 3DS firmware version", "Yes -> Firmware >= 11.4" & @CRLF & "No -> Firmware <= 11.3" & @CRLF & _
			"Cancel -> Go back" & @CRLF & @CRLF & "Lazstream will remember your answer so you won't see this message again.")
	Select
...
Func LogStart()
	...
	If $iLogLevel > 0 Then
		If $iLogConsole == 0 Or $iLogConsole == 2 Then
			...
			FileWriteLine($hFile, "  _                  _                            ")
			FileWriteLine($hFile, " | |    __ _ _______| |_ _ __ ___  __ _ _ __ ___  ")
			FileWriteLine($hFile, " | |   / _` |_  / __| __| '__/ _ \/ _` | '_ ` _ \ ")
			FileWriteLine($hFile, " | |__| (_| |/ /\__ \ |_| | |  __/ (_| | | | | | |")
			FileWriteLine($hFile, " |_____\__,_/___|___/\__|_|  \___|\__,_|_| |_| |_|")
...
Func CheckIfStreaming($sRemoteIP)
	...
	While $bRecieved == False
		...
		ElseIf $dRecv=="" And TimerDiff($hTimerTimeout)>=$iTimeout And $iRet<>-1 Then
			MsgBox($MB_ICONERROR, $sConnectionError, "Could not receive the stream from your console."&@CRLF&@CRLF&"This usually happens if a firewall is blocking Lazstream"& _
			" or if the streaming port is set incorrectly. Try to allow Lazstream in your firewall, running it as admin and double check the streaming port in the advanced" & _
...
Func CheckForUpdates()
	If StringLeft($sVersion, 3) <> "git" And IniRead($sFname, $sSectionName, $aIniSections[17], 0) == 0 Then
		Local $sLatestFullVer = _GetGithubLatestReleaseTag("RattletraPM", "Lazstream")
		Local $sLatestVer = ReturnDecNumber($sLatestFullVer)

		If @error <> -1 And $sLatestVer > ReturnDecNumber($sVersion) Then
			If Not IsDeclared("iMsgBoxAnswer") Then Local $iMsgBoxAnswer
			$iMsgBoxAnswer = MsgBox(67, "New stable version available!", "A new stable version of Lazstream is now available for download!" & @CRLF & @CRLF & _
					"Current version: " & $sVersion & @CRLF & "Latest version: " & $sLatestFullVer & @CRLF & @CRLF & "Do you want to visit Lazstream's GitHub Releases page?" & @CRLF & @CRLF & _
					"(Clicking on Cancel will disable Lazstream's auto update feature by never checking for newer versions ever again)")
			Select
				Case $iMsgBoxAnswer = 6 ;Yes
					ShellExecute("https://github.com/ouylaz/LazStream/releases")
...
Func AboutScreen()
	...
	GUICtrlSetData(-1, "Made by ouylaz,"&@CRLF&@CRLF& "This software and its source code are provided free of charge under the GPL-3.0 License. See License.txt for the full license,"&@CRLF&@CRLF& "Credits: ouylaz developer/beta testers icons Trinsid, icones8.fr,"&@CRLF&@CRLF& "Also if you want to support Lazstream's development, you can join discord server https://bit.ly/3YM4tul but are very appreciated!")
				...
				If $dNTRHash == "0xD707E3BFB90C512DCED1225223BE830F" Then
					...
					If $iMsgBoxAnswer = 6 Then
						If $iPort > 0 And $iPort <= 65535 Then
							...
							If $iNTRRet==1 Then
								If Not IsDeclared("iMsgBoxAnswer") Then Local $iMsgBoxAnswer
								$iMsgBoxAnswer = MsgBox(36,"Change port?","Do you also want to change Lazstream's listening port to the one you just patched in NTR?" & @CRLF & @CRLF & "(If unsure, click Yes)")
...
Func ApplyAdvSettings()
	...
	If $bError=False Then
		...
		If $iLogConsole <> $iLogConsoleRead Then
			MsgBox(64, "Info", "Changes to the logging output will take effect after restarting Lazstream.")
